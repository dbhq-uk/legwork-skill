---
name: legwork
description: Use when the user needs multi-source research with citation tracking, evidence persistence, and structured report generation. Triggers on "legwork", "deep research", "comprehensive analysis", "research report", "compare X vs Y", "analyze trends", or "state of the art". Not for simple lookups, debugging, or questions answerable with 1-2 searches.
---
...
+-- $LEGWORK_DEFAULT_MODE set? --> use it
+-- Otherwise -------------------------> standard
```
...
**Counts as "user named a mode":** the request contains the literal word `quick`, `standard`, `deep`, `ultradeep`, or an equivalent like "quick scan", "ultradeep mode", "do a deep dive". `"deep research"` on its own is an invocation phrase for this skill, not a mode choice, and does NOT select deep mode - fall through to the default.
...
Wrapper path: `python3 ${CLAUDE_SKILL_DIR}/scripts/bd_search.py`. On exit code 2 (auth/quota), tell the user to run `brightdata login` - don't retry. Every Bright Data call costs money; never use it where a built-in would have worked.

---

## Token Efficiency Policy

Legwork burns tokens in three places: scraped page content, subagent transcripts, and report prose. Control all three:
...
**Scripts** (all stdlib-only, no virtualenv; run with any `python3` >= 3.9):
...
| `source_evaluator.py score --jsonl-file [path]` | Credibility scoring (batch); user tiers from `~/.legwork/domains.json` |
...
- `$LEGWORK_OUTPUT` if set, else `<git-root>/docs/research/`, else `$PWD/docs/research/` when not in a git repo.
