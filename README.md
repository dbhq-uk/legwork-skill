...
**And the answer comes back in the conversation**, not only as a path - explained
in plain words rather than pasted: the receipt line, the answer in a sentence or
two, each finding with its confidence band, then the file for the full text and
the evidence log. A handful of short paragraphs, whatever the level.
