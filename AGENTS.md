...
- TypeSafe's Jev is **optional**, like Bright Data: `fetch.py --relevant` does nothing without `TYPESAFE_API_KEY`, a failed call never fails a fetch, and it is called only when `--find` missed - ranking every page would print more text than the whole-page reads it replaces.
