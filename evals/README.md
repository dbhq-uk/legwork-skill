...
| [2026-09-22](runs/2026-09-22-perplexity-comparator.md) | A real legwork report against `perplexity/sonar-deep-research`, twice | Third arm. The paid model missed the answer entirely and recommended a route that cannot do what was asked - at default parameters, and again at high reasoning effort and high search context, where it read a near-disjoint set of sources (2 of 19 shared) and reached the same wrong shape of answer. It also exposed two real defects: the blocked-source ladder escalated transport and never source, so a document behind a policy 403 was recorded as unreachable while a copy opened on the free rung; and a refusal was classified after the body was parsed, so a blocked PDF reported "content type this cannot read" wherever pdftotext was absent |
| [2026-09-24](runs/2026-09-24-rewrite.md) | Sonnet 5, three arms: no skill, `main`, the instruction rewrite | All six cases. The rewrite verified 91 quotes against page text to `main`'s 13 and logged 21 refusals to 4, because subagent returns now carry the page-text path and `main`'s were logged by hand. It cost 65% more and cited as many unopened pages - one report widened a comparison on eight search snippets. Neither version produced the could-not-answer shape on case 4: both answered the neighbouring questions instead |
| [2026-09-24 (partial answer)](runs/2026-09-24-partial-answer.md) | Sonnet 5, case 4 only, three runs | The new `## What can be said instead` shape was written on the first draft in all three runs. Two were then refused by the Stop hook, which was grading with `main`'s gate - an eval of a branch that changes `scripts/` must point `LEGWORK_SKILL_DIR` at the branch. The clean run shipped the shape, led the chat with the non-answer, and satisfied all eight of case 4's rewritten expectations |
| [2026-09-24 (matrix rows)](runs/2026-09-24-matrix-snippet-rows.md) | Sonnet 5, banks prompt, one run; plus today's eleven reports replayed | A matrix row resting only on search results now fails at standard. Replayed, it catches exactly the three rows it was built for. Live, it failed four rows on the first gate run; the run rendered each bank's own page and one of them changed the headline. Paid pages were logged `brightdata` for the first time. Found, not fixed: the orchestrator paraphrased the subagent brief's `fetch.py` command away, so 103 subagent fetches reached the log without page text |
| [2026-09-24 (brief script)](runs/2026-09-24-brief-script.md) | Sonnet 5, banks prompt, two runs | `brief.py` writes the subagent brief. Subagent `fetch.py` calls went from 4 to 59 and 16, and quotes checked against page text from 0 to 19 and 12. Run 1 passed the brief as `$(cat file)` and the subagents recovered by reading the file; with one sentence saying the prompt is text, run 2 put the full brief in every prompt |
